<div>
    <form wire:submit.prevent="submitReservation">
        <div>
            ...
            @error('name') <span style="color:#FF0000">{{$message}}</span> @enderror
        </div>

        <div>
            <label>Number of Guests:</label>
            <input wire:model="nb_guests" type="number" name="nb_guests">
            @error('nb_guests') <span style="color:#FF0000">{{$message}}</span> @enderror
        </div>

        <div>
            <label>Date:</label>
            <input wire:model="date" type="date" name="date">
            @error('date') <span style="color:#FF0000">{{$message}}</span> @enderror
        </div>
